...
The catalog now contains **27 Plant presets and 22 Rock presets**: 14 vegetation and 14 rock additions. The 16 original Tree presets remain available separately.
...
Select or reset a biome to use its expanded mixture. Opening an existing project preserves its saved mixture, dimensions, painted masks, and authored assets. Existing asset definitions remain version 1; projects remain version 2 with version 1 project support. New biome compositions use biomeRevision 2. Older app builds cannot load the new forms.

## Added presets

| Family | Vegetation |
| --- | --- |
| Forest | Berry Thicket, Wood Sorrel Carpet, Moss Cushion, Young Pine, Fallen Log |
...
Six additional plant forms provide grass blades/seed heads, conifer branch tiers and needles, cactus arms, succulent rosettes, low cushions, and fallen wood. Rock adds closed slabs, connected overlapping outcrop members, and scree clusters. Every form has three decreasing LODs and a ground-level pivot. The original preset IDs and their generation paths are retained.
...
- `node scripts/runtime-catalog.mjs`: all 49 preset selectors, new authored-asset round trips, real browser GLBs, LOD galleries, and four biomes in both appearances. Requires a Vite server at `EZ_TEST_URL` (default port 5184).
