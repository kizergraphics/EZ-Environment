...
GLB and environment-pack exports embed standard PBR materials, procedural cactus/fallen-wood LODs, and highest-detail static biome grove instances. Photographic foliage variants use the same LOD sources as the viewport. Terrain color/roughness blends are baked into four bounded 1024-pixel tiles; repeating normal textures retain fine relief through a second UV set. These portable materials are shared by preview and export. Contact shading, sky, fog, exposure, shadows, and animated wind require receiving-renderer setup. Environment manifests include detached appearance/lighting metadata and explicit limitations under `presentation`; their README repeats these limitations. Existing pack format version 1 remains compatible. See the [Unity guide](../unity/README.md) for verified material import and foliage setup.
