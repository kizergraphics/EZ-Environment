 ...
 */
export function setupUI(tree, environment, renderer, scene, camera, orbitControls, initialPreset, onTreeChanged = () => {}) {
  ...
  let lastGoodTree=cleanTreeDefinition(tree);
  const notifyTreeChanged=()=>{lastGoodTree=cleanTreeDefinition(tree);onTreeChanged();};
  ...
  const onChange = () => {
    try{
    const t0 = performance.now();
    const prepared=prepareTreeProject(cleanTreeDefinition(tree),{options:lastGoodTree});commitTreeProject(tree,prepared);
    lastBuildMs = performance.now() - t0;
    if (previewLevel > 0) {
      applyLODPreview();
    }
    ...
    updateInfoDisplays();
    notifyTreeChanged();
    }catch(error){tree.options.copy(lastGoodTree);applyTreeTextures(tree);refreshAllControls();window.__EZ_ENVIRONMENT__?.studio.status(`Tree change not applied: ${error.message}`,true);}
  };
  ...
  const savePresetBtn = createButton('Save Preset', 'document', () => {
    ...
    const json = JSON.stringify(cleanTreeDefinition(tree), null, 2);
  ...
  const exportPngBtn = createButton('Export PNG', 'photo', async ({ setStatus }) => {
    ...
    tree.traverse(o => treeObjects.add(o));
    scene.traverse(o => visibility.set(o, o.visible));
    ...
    scene.traverse((o) => {
      if (o.name === 'Skybox') {
        materialSides.set(o.material, o.material.side);
        o.material.side = THREE.FrontSide;
      } else if (o.isMesh && !treeObjects.has(o)) {
        o.visible = false;
      }
    });
    tree.visible = true;
  ...
  newFileInput.addEventListener('change', function (event) {
    ...
    if (file) {
      ...
      reader.onload = function (e) {
        try {
          if(file.size>8*1024*1024)throw new Error('Tree preset exceeds 8 MB.');
          const prepared=prepareTreeProject(JSON.parse(e.target.result),tree);commitTreeProject(tree,prepared);
          if (previewLevel > 0) {
            applyLODPreview();
          }
          refreshAllControls();
          ...
          window.__EZ_ENVIRONMENT__?.studio.status(`Could not open tree preset: ${error.message}`,true);
        }
      };
  ...
  return {refresh(){lastGoodTree=cleanTreeDefinition(tree);previewLevel=0;lodButtons.forEach((b,i)=>b.classList.toggle('active',i===0));refreshAllControls();}};
}
